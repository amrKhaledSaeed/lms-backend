...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('t_a_droppeds', function (Blueprint $table) {
            $table->dropForeignIdFor(User::class);
        });

        // Add class_id column if it doesn't exist
        if (! Schema::hasColumn('t_a_droppeds', 'class_id')) {
            Schema::table('t_a_droppeds', function (Blueprint $table) {
        ...
        DB::table('t_a_droppeds')->whereNotExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('classes')
                ->whereColumn('classes.id', 't_a_droppeds.class_id');
        })->delete();
