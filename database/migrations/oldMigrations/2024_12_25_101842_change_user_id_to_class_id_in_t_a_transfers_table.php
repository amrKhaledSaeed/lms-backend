...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('t_a_transfers', function (Blueprint $table) {
            $table->dropForeignIdFor(User::class);
        });

        // Add class_id column if it doesn't exist
        if (! Schema::hasColumn('t_a_transfers', 'class_id')) {
            Schema::table('t_a_transfers', function (Blueprint $table) {
        ...
        DB::table('t_a_transfers')->whereNotExists(function ($query) {
            $query->select(DB::raw(1))
                ->from('classes')
                ->whereColumn('classes.id', 't_a_transfers.class_id');
        })->delete();
